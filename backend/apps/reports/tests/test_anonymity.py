...
from apps.core.factories import UserFactory, SecurityFactory, AnonymousReportFactory, DepartmentFactory
...
def test_anonymous_report_does_not_expose_reporter():
    ...
    report = AnonymousReportFactory(department=department, assigned_to=security)
    ...
    # Security user (the assigned responder — Phase 14: report visibility
    # is assignment/department scoped, not a blanket admin-tier grant)
    # fetches the report.
    client = APIClient()
